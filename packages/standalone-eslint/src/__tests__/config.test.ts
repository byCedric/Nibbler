...
describe('defaultConfig', () => {
  it('uses bundled parser settings', () => {
    ...
    expect(defaultConfig).toHaveProperty(
      'parserOptions',
      expect.objectContaining({
        // All files should be treated as modules by default
        sourceType: 'module',
...
describe(getLinterConfig, () => {
  ...
  it('does not overwrite parser from custom config', () => {
    expect(getLinterConfig('file.ts', { parser: 'espree' })).toHaveProperty(
      'parser',
  ...
  it('does not overwrite parserOptions from custom config', () => {
    expect(getLinterConfig('file.ts', { parserOptions: { something: 'else' } })).toHaveProperty(
      'parserOptions',
      expect.objectContaining(defaultConfig.parserOptions)
    );
  });
});
